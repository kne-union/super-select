...
表格形式的选择组件，适用于需要展示多列数据的选择场景。基于 `@kne/table-page`，支持普通列表与树形（`tree` / `treeList`）选择；桌面端滚动时表头固定，横向滚动条吸附在可视区域底部。
...
- `tree` / `treeList` 能力透传自 `@kne/table-page`（展开收起、缩进、半选、移动端面包屑等）
